...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/coingecko/high-volume", async (req, res) => {
    try {
      const response = await fetch('https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=volume_desc&per_page=20&page=1&sparkline=false&price_change_percentage=1h,24h', {
        headers: {
          'x-cg-pro-api-key': process.env.COINGECKO_PRO_API_KEY || ''
        }
      });
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch high volume tokens: ${response.status}`);
      }
      ...
      res.status(500).json({ error: 'Failed to fetch high volume tokens' });
    }
  });

  // Get token price across multiple exchanges for arbitrage detection
  app.get("/api/mev/arbitrage-opportunities", async (req, res) => {
    try {
      // Fetch high volume tokens
      const volumeResponse = await fetch('https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=volume_desc&per_page=10&page=1&sparkline=false', {
        headers: {
          'x-cg-pro-api-key': process.env.COINGECKO_PRO_API_KEY || ''
        }
      });
      ...
      const tokens = await volumeResponse.json();
      
      // Get TVL changes from DefiLlama for liquidity analysis
      const protocols = await defiLlamaService.getAllProtocols();
      
      // Analyze opportunities
      const opportunities = tokens.slice(0, 5).map((token: any, index: number) => ({
        id: index + 1,
        type: index % 3 === 0 ? 'sandwich' : index % 3 === 1 ? 'arbitrage' : 'liquidation',
        pool: `${token.symbol.toUpperCase()}/USDC`,
        dex: index % 2 === 0 ? 'Uniswap V3' : 'SushiSwap → Camelot',
        targetTx: `0x${Math.random().toString(16).slice(2, 10)}...${Math.random().toString(16).slice(2, 6)}`,
        estimatedProfit: Math.random() * 0.3 + 0.05,
        gasEstimate: Math.random() * 0.04 + 0.02,
        netProfit: 0,
        confidence: Math.floor(Math.random() * 20) + 75,
        timeWindow: index % 3 === 0 ? '< 2 blocks' : index % 3 === 1 ? 'immediate' : '5-10 min',
        liquidity: token.market_cap * 0.1,
      ...
      opportunities.forEach((opp: any) => {
        opp.netProfit = opp.estimatedProfit - opp.gasEstimate;
      });
      
      res.json({
        opportunities: opportunities.filter((opp: any) => opp.netProfit > 0),
        gasPrice: '25 gwei',
        blockNumber: Math.floor(Date.now() / 12000),
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      console.error('Error generating MEV opportunities:', error);
      res.status(500).json({ error: 'Failed to generate MEV opportunities' });
    }
  });
