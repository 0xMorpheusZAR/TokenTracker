...
- January 23, 2025: Added OTHERS/BTC ratio illustration to Altseason Dashboard - Created new API endpoint `/api/altseason/others-btc-ratio` that calculates the total altcoin market cap (excluding Bitcoin) vs Bitcoin market cap ratio, displays current ratio with BTC dominance percentage, shows historical data with 7-day and 30-day moving averages, includes critical level indicators (Extreme Greed at 1.5, Strong Altseason at 1.2, Equilibrium at 1.0, BTC Dominance at 0.6), uses purple/indigo color scheme to differentiate from ETH/BTC ratio chart, provides market status interpretation based on ratio levels
```
