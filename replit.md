...
- January 23, 2025: Built comprehensive "Top Performers vs ETH" feature with premium styling - Added new API endpoint /api/altseason/altcoins-performance-eth that fetches Top 50 Altcoins and calculates performance relative to Ethereum for 7D/30D/90D timeframes, implemented complete React Query integration with separate timeframe state management, created visually stunning UI section with indigo/blue gradient theme and glass morphism effects matching Bitcoin performance section, includes BloFin trading links and TradingView chart analysis modals for each coin, displays ETH outperformers with premium badges and animations
- January 23, 2025: Added Monte Carlo simulation section for Top 5 ETH outperformers - Implemented statistical price projections using 100 simulations over 30 days with Geometric Brownian Motion, calculates bearish (10th percentile), base (50th percentile), and bullish (90th percentile) scenarios based on historical volatility and performance drift, displays 5 sample simulation paths with purple gradient visualization, includes real-time price targets with percentage changes for each scenario
- January 23, 2025: Created dedicated Monte Carlo tab next to Analysis tab for enhanced user experience - Moved Monte Carlo simulation from Overview tab to its own dedicated tab for better organization, updated navigation structure to include 4 main tabs: Overview, Analysis, Monte Carlo, and Learn
- January 23, 2025: Fixed small cryptocurrency price display in Monte Carlo simulations - Updated Y-axis formatter to properly show 8 decimal places for micro-cap tokens like BONK ($0.00003384), improved legend and disclaimer spacing with dedicated background cards and larger gaps, enhanced tooltip price formatting for all price ranges, reorganized legend section into three separate cards for better visual hierarchy
- January 23, 2025: Enhanced Monte Carlo simulations with live percentage projections - Reordered chart lines with Bullish on top, Most Likely in middle, Bearish at bottom for intuitive visualization, removed "Now" text from reference line for cleaner display, added three interactive mini dashboards showing 30-day percentage increases for each scenario (Bullish/Most Likely/Bearish), displays both percentage change and final price projections with proper decimal formatting for all token price ranges
- January 23, 2025: Finalized Monte Carlo visualization refinements - Removed all text next to green reference lines across all simulations for cleaner presentation, adjusted X-axis text positioning lower (dy: 10) for better visual spacing, green dashed reference line now displays without any labels as requested
- January 23, 2025: Enhanced Monte Carlo simulations to actively monitor and dynamically reorder based on Top 5 ETH outperformers - Simulations automatically update when coins enter/exit the top 5, added visual ranking badges (#1-#5) with gold/silver/bronze styling, implemented "Live Updates" indicator header, and optimized component with React.memo for zero-lag performance
```
