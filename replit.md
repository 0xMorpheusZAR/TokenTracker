...
- January 23, 2025: Added OTHERS/BTC ratio illustration to Altseason Dashboard - Created new API endpoint `/api/altseason/others-btc-ratio` that calculates the Total Market Capitalization Excluding Top 10 Altcoins And BTC divided by BTC Market Cap ratio, this represents smaller altcoins (rank 11+) relative to Bitcoin, displays current ratio with BTC dominance percentage, shows historical data with 7-day and 30-day moving averages, includes critical level indicators (Extreme Small Cap Season at 0.5, Strong Altseason at 0.4, Altseason Start at 0.3, Neutral at 0.2), uses purple/indigo color scheme to differentiate from ETH/BTC ratio chart, provides market status interpretation for small cap market conditions relative to Bitcoin dominance
```
